...
class TestPetManagement:
    ...
    def test_request_access_success(self, client, mock_db, regular_user_token, admin_pet):
        ...
        response = client.post(
            f"/api/pets/{admin_pet['_id']}/request-access", headers={"Authorization": f"Bearer {regular_user_token}"}
        )
    ...
    def test_request_access_already_has_access(self, client, mock_db, regular_user_token, test_pet):
        ...
        response = client.post(
            f"/api/pets/{test_pet['_id']}/request-access", headers={"Authorization": f"Bearer {regular_user_token}"}
        )
    ...
    def test_get_access_requests_owner_only(self, client, mock_db, regular_user_token, test_pet):
        ...
        db["pets"].update_one(
            {"_id": test_pet["_id"]},
            {"$addToSet": {"access_requests": {"username": "requester", "requested_at": datetime.utcnow()}}},
        )

        response = client.get(
            f"/api/pets/{test_pet['_id']}/access-requests", headers={"Authorization": f"Bearer {regular_user_token}"}
        )
